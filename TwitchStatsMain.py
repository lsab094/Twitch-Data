...
#select returns tuple, can eb indexed etc
#where column1 = ? AND colum2 = ?""", (value1, value2)
#syntax/order of query
#rank, desne rnak, row num
#matplotlib line and bar graphs, numpy,

z = np.linspace(2,10,5) #floats, good for plots, 2 to 10 with 5 values (2, 4, 6, 8, 10)
z = np.array([ [1,2,3], [4,5,6] ]) #put list(s) inside
#accessing values is the same, ex. z[0:2] + z[-1]
a = np.sum(z) #sum, prod, mean, min, max
b = z < 3 #returns true/false for each value
c = z[z < 3] #only elements less than 3
d = z * z #equal num of elements, add/multiply each index together or by other number
e = z.T #transpose, switch rows and columns
z = np.sort(z) #sort in order

# create table
# sql = """
#     CREATE TABLE StreamerTable(
#         Channel TEXT,
#         Watch_time INTEGER,
#         Stream_time INTEGER,
...
# another way to open
# with open('StreamerData.csv', 'r', encoding='utf-8', errors='ignore') as file1:
#     for row in file1:
#         curs.execute("INSERT INTO StreamerTable VALUES (?,?,?,?,?,?,?,?,?,?,?)", row.split(","))
#         con.commit()
# con.close()